...
alter table employee add constraint FKghecv11ypswk5w7mmcof2dscg foreign key (reports_to) references employee (emp_id) on delete no action;
 alter table employee add constraint FK3046kvjyysq288vy3lsbtc9nw foreign key (role_id) references role (role_id) on delete cascade;
 alter table leave_entitled add constraint FKqc3wb4pq044fmqp7tfvcs7k0q foreign key (employee_id) references employee (emp_id) on delete cascade;
 alter table leave_entitled add constraint FKek3eqh31l3wwfiiwn2ptwpuho foreign key (leave_type_id) references leave_type (leave_type_id) on delete cascade;
 ...
 alter table role_leave_type add constraint FKqp007mrnfmbrx74xc3ok5cexc foreign key (leave_type_id) references leave_type (leave_type_id) on delete cascade;
 alter table role_leave_type add constraint FKqx6r7w4cseubxoufk7nudc4ei foreign key (role_id) references role (role_id) on delete cascade;